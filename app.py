...
async def product_owner_review(task_id: str, request: Request):
    ...
    return await generic_workflow_review(
        task_id,
        request,
        review_type='product_owner_review'
    )
...
async def generic_workflow_review(
    ...
    if saved_state:
        ...
        if review_type == 'product_owner_review':
        ...
        elif review_type == "test_cases_review":
        ...
        elif review_type == "qa_testing_review":
        ...
        state = None
        async for event in graph.astream(None, thread, stream_mode="values"):
            print(f"{review_type.replace('_review', '').capitalize()} Review Event Received: {event}")
            state = event
